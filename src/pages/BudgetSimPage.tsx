...
const getScale = () => {
  ...
  return Math.max(0.12, scale * 0.985)
}

export default function BudgetSimPage() {
  const [budget, setBudget] = useState(300)
  const [hasStarted, setHasStarted] = useState(false)
  const [budgetDraft, setBudgetDraft] = useState('300')
  ...
  const insight = useMemo(() => {
    ...
    const spentAfterChoice = calculateSpent(selections)

    return getStepInsight({
      budget,
      spentBeforeChoice: spentAfterChoice - currentOption.price,
      currentNode,
      chosenOption: currentOption,
      selections,
    })
  }, [budget, currentNode, currentOption, selections])

  const hintCopy = currentOption
    ? [
        `剩余 ${formatMoney(remaining)}`,
        insight?.pressureLabel ?? '',
        shortenText(insight?.watchout ?? '', 24),
      ]
        .filter(Boolean)
        .join(' · ')
    : '先选一个方案，我会提示预算压力。'
  ...
  const handleStart = () => {
    ...
    const nextBudget = Number.isFinite(parsed) ? Math.max(1, parsed) : 300
  ...
  return (
    <main className="budget-sim-root">
      <section className="stage" style={{ transform: `scale(${scale})` }}>
        ...
        {!hasStarted ? (
          <section className="budget-picker sketch-card start-picker">
            <h3>先设置你的预算</h3>
            ...
            <button type="button" className="close-picker" onClick={handleStart}>
              开始进入选择
            </button>
          </section>
        ) : (
          <>
            ...
            <section className="progress-box sketch-card">
              ...
              <strong>
                {Math.max(currentStep + 1, completedCount)} / {scriptNodes.length}
              </strong>
